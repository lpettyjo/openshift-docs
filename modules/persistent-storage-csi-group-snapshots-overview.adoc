...
Container Storage Interface (CSI) volume group snapshots needs to be supported by the CSI driver. {product-title} supports CSI volume group snapshots by default. {rh-storage} supports volume group snapshots.
