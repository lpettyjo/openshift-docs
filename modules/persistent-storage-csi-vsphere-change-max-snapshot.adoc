...
[role="_abstract"]
To customize vSphere snapshot retention limits, configure the maximum number of snapshots per volume globally or for specific datastore types to balance capacity and performance.
...
----
+
Where under "Snapshot", `global-max-snapshots-per-block-volume` is now set to 10.
