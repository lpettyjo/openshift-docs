...
[role="_abstract"]
Container Storage Interface (CSI) volume clones are point-in-time duplicates of existing persistent volumes. Cloning is more efficient than snapshots to create a copy. Specify an existing persistent volume claim (PVC) as the source to clone volumes in a namespace using supported CSI drivers.
