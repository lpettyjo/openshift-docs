...
If a resize request fails or remains in a pending state, you can try again by entering another smaller resize value in `.spec.resources.requests.storage` for the persistent volume claim (PVC). The smaller value should be smaller than the original volume size.
