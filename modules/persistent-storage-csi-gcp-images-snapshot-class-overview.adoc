...
For Google Cloud Platform (GCP) persistent disk (PD) storage  CSI, there is a non-default `VolumeSnapshotClass`, `csi-gce-pd-vsc-images`, that includes the `snapshot-type`: `images` parameter. When using KubeVirt, this images snapshot class allows you overcome the six VMs per hour restriction, so that you can create VMs from "golden images". 
