...
[role="_abstract"]
Persistent volumes (PVs) are cluster-wide storage resources defined by capacity, access modes, mount options, reclaim policies, and lifecycle phases. Each storage backend supports different access mode combinations, and volumes transition through phases (Available, Bound, Released, Failed) affecting claim availability.
...
* `spec.storage`: The amount of storage available to the volume.

* `spec.accessModes`: The access mode, defining the read/write and mount permissions.

* `spec.persistentVolumeReclaimPolicy`: The reclaim policy, indicating how the resource should be handled once it is released.
