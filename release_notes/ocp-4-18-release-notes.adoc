...
==== Always honored persistent volume reclaim policy (Technical Preview)
...
{product-title} supports volume group snapshots by default. {rh-storage} supports volume group snapshots.
...
==== GCP Filestore supporting WIF is generally available
In {product-title} 4.18, Google Compute Platform (GCP) Filestore Container Storage Interface (CSI) storage supports Workload Identity Federation (WIF). This allows users to access Google Cloud resources using federated identities instead of a service account key. For {product-title} 4.18, this feature is generally available.
...
// and xref:????link to appropriate install area???.
