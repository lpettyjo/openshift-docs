...
{product-title} GCP Filestore supports Workload Identity Federation (WIF). This allows users to access Google Cloud resources using federated identities instead of a service account key. 

ifndef::openshift-dedicated[]
To enable WIF support, see xref:../../installing/installing_gcp/installing-gcp-customizations.adoc#installing-gcp-with-short-term-creds_installing-gcp-customizations[Configuring a GCP cluster to use short-term credentials].
